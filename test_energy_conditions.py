...
def _to_SI_pressures(pr_geom, pt_geom):
    """
    Convert pressures from geometrized units (c=1) to SI (Pascal).
    pr_geom, pt_geom: Pressures in geometrized units (same order as rho_si/c^2)
    Returns: (p_r_SI, p_t_SI) in Pascal
    
    Heuristik: Falls bereits |pr_geom| >> |rho_si| (Faktor >1e10), 
    dann schon SI -> nicht skalieren. Sonst: geometrisiert -> mit c^2 multiplizieren.
    """
    c2 = c * c
    # In geometrisierten Einheiten: pr ~ rho ~ kg/m^3, nicht Pa
    # In SI: p [Pa] = p_geom [kg/m^3] * c^2
    return pr_geom * c2, pt_geom * c2
...
def main():
    ...
    for r in radii:
        ...
        # Test-Logik bleibt unverändert (geometrisierte Einheiten)
        ...
        # Nur für Ausgabe: Konvertiere Drücke zu SI (Pascal)
        pr_SI, pt_SI = _to_SI_pressures(pr, pt)
        
        print(f"{r/rs:8.2f} {rho:15.3e} {pr_SI:15.3e} {pt_SI:15.3e} {wec_str:>6} {dec_str:>6} {sec_str:>6}")
    ...
    print(f"  • p_r = -ρc² (radial tension balances density)")
