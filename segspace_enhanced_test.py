...
# -*- coding: utf-8 -*-
"""
segspace_enhanced_test.py (built-in Seg model incl. deltaM mode, FIXED)
Strict-but-flexible test runner for Segmented Spacetime vs GR/SR baselines.

Features
--------
- CSV ingest with explicit z-provenance (prefer z over freqs if --prefer-z)
- Strong-row detection for orbit mode (a_m [m], e, f_true_deg, M_solar)
- Baselines: GR, SR, GR*SR
- Built-in Seg model (--seg-mode {hint,grsr,deltaM})
  * hint  : (1+z_seg)=(1+z_geom_hint)*(1+z_SR)-1 ; fallback=GR*SR
  * grsr  : z_seg=GR*SR
  * deltaM: (1+z_seg)=(1+ z_GR_scaled )*(1+z_SR)-1  with  z_GR_scaled = z_GR*(1+ΔM_frac)
            Δ(M)=(A·e^(−α·r_s)+B)·norm(log10 M),  A,B in percent
- Per-model residuals with median/mean/max |Δz|
- Optional plots + JUnit + full debug CSV
- SHA256 of input CSV printed for reproducibility
- Verbose WARN logs when SR cannot be computed

Usage
-----
  python segspace_enhanced_test.py --csv real_data_30_segmodel.csv --prefer-z --seg-mode deltaM \
...
def z_special_rel(v_tot_mps, v_los_mps=0.0):
    ...
    if v_tot_mps is None or not math.isfinite(v_tot_mps) or v_tot_mps < 0:
        return float('nan')
    beta = min(v_tot_mps / c, 0.999999999999)
...
def evaluate(df, prefer_z: bool, seg_mode: str, outdir: Path, make_plots: bool, write_junit: bool,
    ...
    for idx, r in df.iterrows():
        ...
        # velocities (sanitize v_los -> 0.0 if NaN/None)
...
def main():
    ap = argparse.ArgumentParser(description="Enhanced tests for segmented spacetime vs GR/SR baselines")
    ...
    ap.add_argument("--seg-mode", type=str, default="hint", choices=["hint","grsr","deltaM"],
                    help="Built-in Seg model: 'hint' uses z_geom_hint; 'grsr' equals GR*SR; 'deltaM' applies Δ(M) scaling")
    # deltaM params
