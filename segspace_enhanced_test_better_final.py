...
# -*- coding: utf-8 -*-
"""
segspace_enhanced_test_better.py (Improved version with velocity fixes + sanity checks)
...
- Physics sanity checks for radii, velocities, ΔM parameters
- Extra logging for heuristic corrections
- Robust mean for outlier-resistant statistics
...
def sanity_check_row(case, M_c, r_eff, v_tot, v_los):
    issues = []
    if M_c > 0 and r_eff is not None and math.isfinite(r_eff):
        rs = 2*G*M_c/c**2
        if r_eff <= rs:
            issues.append(f"r_eff <= r_s ({r_eff:.3e} <= {rs:.3e})")
        if r_eff < 1e6:  # < 1000 km
            issues.append(f"r_eff suspiciously small ({r_eff:.3e} m)")
    if v_tot is not None and math.isfinite(v_tot):
        if v_tot > c:
            issues.append(f"v_tot > c ({v_tot:.3e} m/s)")
        if abs(v_tot) < 1:
            issues.append(f"v_tot near zero ({v_tot:.3e} m/s)")
    if v_los is not None and math.isfinite(v_los):
        if abs(v_los) > c:
            issues.append(f"v_los > c ({v_los:.3e} m/s)")
    if issues:
        print(f"[CHECK] {case}: " + "; ".join(issues))
    return issues
...
def fix_stellar_velocities(row: dict, category: str) -> tuple[float, str]:
    ...
    if category in ["nearby-stars", "wolf-rayet", "luminous-blue"]:
        if v_tot is not None and v_tot < 0:
            ...
            print(f"[VELFIX] cat={category} orig={v_tot:.1f} -> {v_tot_corrected:.1f} m/s (rad={v_radial}, tan={v_tangential})")
            return v_tot_corrected, f"fixed_negative(v_rad={v_radial:.0f},v_tan={v_tangential:.0f})"
        elif v_tot is None and abs(v_los) > 1000:
            v_radial = abs(v_los)
            v_tangential = 25000
            v_tot_corrected = math.sqrt(v_radial**2 + v_tangential**2)
            print(f"[VELFIX] cat={category} from v_los={v_los:.1f} -> {v_tot_corrected:.1f} m/s")
...
def z_seg_pred_deltam(row: dict, z_gr: float, z_sr: float,
    ...
    if norm in (0.0, 1.0):
        print(f"[DM-NORM] case={row.get('case','?')} norm={norm:.2f}")
    if abs(deltaM_pct) > 50:
        print(f"[DM-VAL] case={row.get('case','?')} ΔM={deltaM_pct:.1f}%")
...
def robust_mean(vec):
    ...
    m = stats.median(vec)
    mad = stats.median([abs(x-m) for x in vec])
    filtered = [x for x in vec if abs(x-m) <= 5*mad]
...
def evaluate(df, prefer_z: bool, seg_mode: str, outdir: Path, make_plots: bool, write_junit: bool,
    ...
    for idx, r in df.iterrows():
        ...
        sanity_issues = sanity_check_row(case, M_c, r_eff, v_tot, v_los)
        ...
        dbg_rows.append({
            **row,
            ...
            "strong": strong,
            "rs_m": (2.0 * G * M_c / (c**2)) if (M_c > 0) else "",
            "deltaM_percent": (dmA * math.exp(-dmAlpha * ((2.0 * G * M_c / (c**2)) if M_c > 0 else 0.0)) + dmB) if seg_mode in ["deltaM", "hybrid"] and (M_c > 0) else "",
            "log10M_norm": (math.log10(M_c) - dataset_logM_min) / (dataset_logM_max - dataset_logM_min) if seg_mode in ["deltaM", "hybrid"] and (M_c > 0) and dataset_logM_min is not None and dataset_logM_max is not None and dataset_logM_max > dataset_logM_min else "",
            "sanity_flags": "|".join(sanity_issues) if sanity_issues else ""
        })
