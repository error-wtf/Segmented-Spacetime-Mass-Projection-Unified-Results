...
Robust single-point evaluation of the SSZ stability criterion with
floating-point safeguards and the corrected mirror term (applied once per
roundtrip).
...
def clean_float(value: float) -> float:
    """Round floats to a deterministic representation for reporting."""
...
def weighted_segment_angles(params: Parameters, resolution: int = 8192) -> np.ndarray:
    ...
    for idx, target in enumerate(targets):
        pos = np.searchsorted(cumulative, target, side="left")
        pos = int(np.clip(pos, 1, resolution - 1))
...
def compute_L(params: Parameters) -> float:
    ...
    values = r_theta(theta, params)
    return integrate_over_theta(values, params)
...
def add_common_overrides(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--alpha", type=float, help="Override alpha")
    parser.add_argument("--eta", type=float, help="Override eta")
    parser.add_argument("--omega", type=float, help="Override omega")
    parser.add_argument("--m", type=int, help="Override mode number m")
    parser.add_argument("--Omega0", type=float, help="Override Omega0")
    parser.add_argument("--epsilon", type=float, help="Override epsilon")
    parser.add_argument("--q", type=float, help="Override q")
    parser.add_argument("--lambdaA", type=float, help="Override lambda_A")
    parser.add_argument("--K", type=int, help="Override K")
    parser.add_argument("--sigma0", type=float, help="Override sigma0")
    parser.add_argument("--R", type=float, help="Override mirror reflectivity")
    parser.add_argument("--Kappa", type=float, help="Override coupling loss")
    parser.add_argument("--phi", type=float, help="Override phi")
    parser.add_argument("--r0", type=float, help="Override r0")
    parser.add_argument("--theta-samples", type=int, help="Override theta sample count")
    parser.add_argument("--segment-mode", choices=["uniform", "weighted"], help="Segment mode")


def apply_overrides(base: Parameters, args: argparse.Namespace) -> Parameters:
    overrides = {field: getattr(args, field) for field in (
        "alpha", "eta", "omega", "m", "Omega0", "epsilon", "q", "lambdaA",
        "K", "sigma0", "R", "Kappa", "phi", "r0", "theta_samples"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SSZ proof check v4")
    parser.add_argument("--input", type=str, help="Optional JSON file with parameters")
    parser.add_argument("--output", type=str, default="d:/extended_results/proof_reports/proof_report_v4.json",
